...
module.exports = function (__dirname, config) {
  ...
  if (!config.hostPort) {
    config.hostPort = 8082
  }
  if (config.systemLog === undefined) {
  ...
  if (config.mode === 'host') {
    ...
    const createConnection = () => {
      ...
      io.on('connect', (d) => {
        ...
        plugLog('Sending init request to Shinobi')
        s.cx({
          f: 'init',
          plug: config.plug,
      ...
      io.on('disconnect', () => {
        plugLog("receive disconnect")
        onDisconnect()
      })
      io.on('error', (error) => {
        plugLog("An error occurred");
        plugLog(error);
        onDisconnect(error);
      })
      io.on('f', (d) => {
  ...
  s.createPythonScriptDaemon = () => {
    if (!config.pythonScript) {
      config.pythonScript = config.dirname + '/pumpkin.py'
    }
    if (!config.pythonPort) {
      config.pythonPort = 7990
    }
